...
from lib.datasets import build_dataset_fold
...
def get_args_parser():
    ...
    parser.add_argument('--data-path', default='./data/imagenet/', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='UCM', choices=['CIFAR', 'IMNET', 'INAT', 'INAT19','UCM','NWPU','AID'],
...
def cm(args):
    ...
    dataset_train, args.nb_classes = build_dataset_fold(is_train=True, args=args)
    cm_total = np.zeros((args.nb_classes, args.nb_classes))  # 混淆矩阵
    ...
    for i in range(5):
        args.fold_idx = i
        dataset_train, args.nb_classes = build_dataset_fold(is_train=True, args=args)
        dataset_val, _ = build_dataset_fold(is_train=False, args=args)
        if args.distributed:
        ...
        if args.resume:
            if args.resume.startswith('https'):
                ...
                checkpoint = torch.load(f"/data1/users/liujie/PycharmProjects/pythonProject/AutoFormer-Scene/AutoFormer/output/UCM_fold{i}/checkpoint.pth", map_location='cpu')
        ...
        retrain_config = None
        args.cfg = f"/data1/users/liujie/PycharmProjects/pythonProject/AutoFormer-Scene/AutoFormer/experiments/subnet/UCM_fold{i}.yaml"
        update_config_from_file(args.cfg)
        ...
        pred_v = model(images)
        pred = torch.argmax(pred_v,dim=1)
        pred_cpu = pred.cpu()
        target_cpu = target.cpu()
        label = range(21)
        cm = confusion_matrix(target_cpu, pred_cpu, labels= label)
        cm_total += cm

    print(cm_total)
    label_table = [f"class {i}" for i in range(21)]
    disp = ConfusionMatrixDisplay(confusion_matrix=cm_total, display_labels=label_table)
    disp.plot(cmap=plt.cm.Blues)
    plt.title('Cumulative Confusion Matrix (5-Fold Cross Validation)')
    plt.show()
